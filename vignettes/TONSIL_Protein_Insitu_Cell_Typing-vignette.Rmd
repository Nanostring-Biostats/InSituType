...
The below command performs unsupervised clustering. We will let the algorithm try to select the optimal number of clusters from a range of plausible options using the `n_clusts` argument and set `reference_profiles` and `reference_sds` as NULL. For protein runs, be sure to set `assay_type = "protein"`. 
...
sup <- insitutype(
  ...
  n_clusts=0, 
  anchors = NULL, 
  reference_profiles = tonsil_reference_profile$mean.ref.profile, 
...
plot(tonsil_protein$xy_coord, pch = 16, cex = .25, asp = 1, cex.main = 0.75,
          main = "Cells in physical space",
     col = cols[unsup$clust], xlab = "X", ylab = "Y", xaxt = "n", yaxt = "n")

plot(tonsil_protein$UMAP, pch = 16, cex = .25, asp = 1, cex.main = 0.75,
